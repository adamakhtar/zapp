defmodule Zapp.Accounts do
  ...
  def get_user_with_account_by_identity!(%Identity{} = identity) do
    query =
      from u in User,
        ...
        where: u.identity_id == ^identity.id,
        preload: [:account]
  ...
  ## Examples
      ...
      iex> register_owner_with_account(%{field: value})
      {:ok, %Identity{}}

      iex> register_owner_with_account(%{field: bad_value})
      {:error, %Ecto.Changeset{}}

  """

  def register_owner_with_account(identity_attrs) do
    Multi.new()
    |> Multi.insert(:identity, Identity.registration_changeset(%Identity{}, identity_attrs))
    |> Multi.insert(:account, Account.changeset(%Account{}, %{}))
    |> Multi.insert(:user, fn %{identity: identity, account: account} ->
      User.changeset(%User{}, %{account_id: account.id, identity_id: identity.id})
    end)
    |> Repo.transaction()
  end
  ...
  def change_identity_registration(%Identity{} = identity, attrs \\ %{}) do
    Identity.registration_changeset(identity, attrs, hash_password: false)
  end
  ...
  defp identity_email_multi(identity, email, context) do
    changeset =
      identity
      |> Identity.email_changeset(%{email: email})
      |> Identity.confirm_changeset()
  ...
  def change_identity_password(identity, attrs \\ %{}) do
    Identity.password_changeset(identity, attrs, hash_password: false)
  end
  ...
  ## Examples
      ...
      iex> deliver_identity_confirmation_instructions(identity, &Routes.identity_confirmation_url(conn, :edit, &1))
      {:ok, %{to: ..., body: ...}}

      iex> deliver_identity_confirmation_instructions(confirmed_identity, &Routes.identity_confirmation_url(conn, :edit, &1))
