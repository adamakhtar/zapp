defmodule ZappWeb.IdentitySettingsControllerTest do
  ...
  describe "PUT /identities/settings (change password form)" do
    test "updates the identity password and resets tokens", %{conn: conn, identity: identity} do
      new_password_conn =
        put(conn, Routes.identity_settings_path(conn, :update), %{
  ...
  describe "PUT /identities/settings (change email form)" do
    @tag :capture_log
  ...
  describe "GET /identities/settings/confirm_email/:token" do
    ...
    test "updates the identity email once", %{conn: conn, identity: identity, token: token, email: email} do
