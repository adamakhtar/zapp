defmodule Zapp.AccountsTest do
  use Zapp.DataCase

  alias Zapp.Accounts

  import Zapp.AccountsFixtures
  alias Zapp.Accounts.{Account, User, Identity, IdentityToken}
  ...
  describe "get_user_with_account_by_identity!/1" do
    ...
    test "raises if no user is found for given identity" do
      identity = identity_fixture()
      assert_raise Ecto.NoResultsError, fn ->
        Accounts.get_user_with_account_by_identity!(identity)
      end
    end
  end

  describe "register_owner_with_account/1" do
    test "creates an account, user and identity with valid params" do
      identity_attrs = valid_identity_attributes(%{email: "test@example.com"})

      {:ok, %{
        account: %Account{} = account,
        user: %User{} = user,
        identity: %Identity{} = identity
      }} = Accounts.register_owner_with_account(identity_attrs)
    ...
    test "validates email and password when given" do
      {:error, :identity, changeset, _} = Accounts.register_owner_with_account(%{email: "not valid", password: "not valid"})
  ...
  describe "register_identity/1" do
    ...
    test "registers identities with a hashed password" do
      ...
      {:ok, identity} = Accounts.register_identity(valid_identity_attributes(email: email))
  ...
  describe "change_identity_registration/2" do
    ...
    test "allows fields to be set" do
      ...
      changeset =
        Accounts.change_identity_registration(
          %Identity{},
          valid_identity_attributes(email: email, password: password)
        )
  ...
  describe "change_identity_password/2" do
    ...
    test "allows fields to be set" do
      changeset =
        Accounts.change_identity_password(%Identity{}, %{
  ...
  describe "confirm_identity/1" do
    setup do
      identity = identity_fixture()
